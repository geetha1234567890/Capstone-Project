pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                script {
                    def dockerCmd = 'docker run -itd --name My-first-container -p 8085:8080 geethgulabrathod/dev:latest'
                    sshagent(['sshkeypair']) {
                        sh "ssh -o StrictHostKeyChecking=no ubuntu@13.126.123.14 ${dockerCmd}"
                    }
                }
            }
        }
